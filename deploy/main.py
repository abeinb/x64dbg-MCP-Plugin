...
from fusion import FusionTools
from fusion2 import FusionTools2
from fusion3 import FusionTools3

class ServerConfig:
    def __init__(self):
        # 支持通过环境变量配置，方便跨机器部署
        # 传输方式：stdio（默认，Trae CN 直接启动）/ streamable-http（独立 HTTP 服务）
        # stdio 模式：Trae CN 启动 main.py，通过 stdin/stdout 通信，连接虚拟机 C++ 插件
        # http 模式：独立运行，监听端口，支持远程连接
        # 跨机器连接虚拟机 x64dbg 时设置 LYSCRIPT_DBG_HOST=虚拟机IP
        self.mcp_service_id = "lyscript_mcp_server_cherry"
        self.mcp_host = os.environ.get("LYSCRIPT_MCP_HOST", "0.0.0.0")
        self.mcp_port = int(os.environ.get("LYSCRIPT_MCP_PORT", "8001"))
        self.mcp_transport = os.environ.get("LYSCRIPT_MCP_TRANSPORT", "stdio")
        self.log_level = os.environ.get("LYSCRIPT_LOG_LEVEL", "INFO")
        self.auto_approve_tools = True
        self.timeout = int(os.environ.get("LYSCRIPT_TIMEOUT", "1800"))
        self.system_prompt = """
...
class PeTools:
    def __init__(self, config: ServerConfig):
        ...
        # C++ 插件地址支持环境变量配置
        # 默认 127.0.0.1:8000（MCP Server 与 x64dbg 同机运行）
        # 若 MCP Server 在本机运行连接虚拟机 x64dbg，设置 LYSCRIPT_DBG_HOST=虚拟机IP
        dbg_host = os.environ.get("LYSCRIPT_DBG_HOST", "127.0.0.1")
        dbg_port = int(os.environ.get("LYSCRIPT_DBG_PORT", "8000"))
        self.dbg_config = Config(address=dbg_host, port=dbg_port)
...
def register_tools(mcp: FastMCP, config: ServerConfig):
    ...
    # ============================================================
    # 注册融合增强工具（集大家所长）
    # 融合 AgentSmithers/x64dbgpy/逆向工程实践的独特优点
    # ============================================================
    # 修复毒舌审查Bug: 原代码传 config（ServerConfig 实例），但 FusionTools.__init__ 期望 petools（PeTools 实例）
    # config 无 dbg/memory/script 等属性，会导致 8 个 fusion 工具初始化即 AttributeError 全部报废
    fusion_tools = FusionTools(petools)
    mcp.tool()(fusion_tools.get_all_registers)
    mcp.tool()(fusion_tools.get_call_stack)
    mcp.tool()(fusion_tools.wait_breakpoint_snapshot)
    mcp.tool()(fusion_tools.set_conditional_breakpoint)
    mcp.tool()(fusion_tools.compare_memory)
    mcp.tool()(fusion_tools.batch_read_memory)
    mcp.tool()(fusion_tools.get_memory_map_modules)
    mcp.tool()(fusion_tools.exec_with_retry)

    # ============================================================
    # 第二轮融合增强工具（融合 ida-pro-mcp / GhidraMCP / CE MCP）
    # ============================================================
    fusion_tools2 = FusionTools2(petools)
    mcp.tool()(fusion_tools2.int_convert)
    mcp.tool()(fusion_tools2.read_string)
    mcp.tool()(fusion_tools2.find_pattern)
    mcp.tool()(fusion_tools2.analyze_function)
    mcp.tool()(fusion_tools2.read_struct)
    mcp.tool()(fusion_tools2.list_threads)
    mcp.tool()(fusion_tools2.batch_set_comments)
    mcp.tool()(fusion_tools2.trace_function_calls)

    # ============================================================
    # 第三轮融合增强工具（深耕 x64dbg 深度能力，针对 VMP脱壳/卡密验证/反调试绕过）
    # 融合 ida-pro-mcp / GhidraMCP / CE MCP 的工具设计思路，底层全部用 x64dbg API 实现
    # ============================================================
    fusion_tools3 = FusionTools3(petools)
    # P0 反调试绕过核心（5个）
    mcp.tool()(fusion_tools3.check_anti_debug_status)
    mcp.tool()(fusion_tools3.bypass_anti_debug_peb)
    mcp.tool()(fusion_tools3.set_memory_access_breakpoint)
    mcp.tool()(fusion_tools3.set_conditional_hardware_breakpoint)
    mcp.tool()(fusion_tools3.exec_x64dbg_command_safe)
    # P1 脱壳侦察+dump+patch（7个）
    mcp.tool()(fusion_tools3.find_oep_by_section_attr)
    mcp.tool()(fusion_tools3.get_pe_full_info)
    mcp.tool()(fusion_tools3.detect_tlscallback)
    mcp.tool()(fusion_tools3.dump_module_memory)
    mcp.tool()(fusion_tools3.set_memory_protect_rw)
    mcp.tool()(fusion_tools3.patch_code_with_assemble)
    mcp.tool()(fusion_tools3.nop_code_range)
    # P2 交叉引用+跟踪+SEH+IAT（6个）
    mcp.tool()(fusion_tools3.get_xrefs_combined)
    mcp.tool()(fusion_tools3.find_xrefs_via_x64dbg)
    mcp.tool()(fusion_tools3.monitor_global_flag_write)
    mcp.tool()(fusion_tools3.trace_into_with_filter)
    mcp.tool()(fusion_tools3.get_seh_chain)
    mcp.tool()(fusion_tools3.scan_iat_and_resolve)
    # P3 便捷工具（2个）
    mcp.tool()(fusion_tools3.bypass_anti_debug_via_runcmd)
    mcp.tool()(fusion_tools3.dump_memory_range)

if __name__ == "__main__":
    config = ServerConfig()

    # stdio 传输时所有输出必须走 stderr，避免干扰 stdout 上的 MCP 协议
    use_stdio = (config.mcp_transport == "stdio")
    ...
    try:
        ...
        if not use_stdio:
            ...
            print(f"[*] MCP(stdio) connecting to x64dbg plugin at "
                  f"{os.environ.get('LYSCRIPT_DBG_HOST', '127.0.0.1')}:"
                  f"{os.environ.get('LYSCRIPT_DBG_PORT', '8000')}", file=sys.stderr)
    except OSError as e:
    ...
    # FastMCP.run() 是同步方法，内部用 anyio.run() 启动事件循环
    # 直接调用即可，不需要 asyncio.run() 包装
